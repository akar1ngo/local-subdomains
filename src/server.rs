...
pub async fn parse_packet(
    ...
    match Message::from_bytes(packet) {
        Ok(message) => {
            handle_query(message, from, socket, is_ipv4, network_config).await?;
        }
        Err(e) => {
...
pub async fn handle_query(
    ...
    if !answers.is_empty() {
        ...
        for record in &answers {
            ...
            if record.record_type() == RecordType::CNAME {
                ...
                if let Some(ipv4_addr) = network_config.ipv4_address
                    && let Ok(a_record) = get_a_record(ipv4_addr)
                {
                ...
                if let Some(ipv6_addr) = network_config.ipv6_address
                    && let Ok(Some(aaaa_record)) = get_aaaa_record(ipv6_addr)
                {
        ...
        let destination = if unicast {
            ...
            SocketAddr::new(IpAddr::V6(MDNS_IPV6), MDNS_PORT)
        };
...
mod tests {
    ...
    fn test_network_config() -> NetworkConfig {
        NetworkConfig {
            ...
            ipv6_address: Some(std::net::Ipv6Addr::new(0x2001, 0xdb8, 0, 0, 0, 0, 0, 1)),
        }
    }
